...
def calculate_priority_score(problem: Dict) -> float:
    ...
    difficulty_multipliers = {"hard": 1.5, "medium": 1.2, "easy": 1.0}
    ...
    priority_score = recency_score * difficulty_multiplier * frequency_penalty
...
def filter_problems(
    ...
    if difficulty:
        filtered = [
            p for p in filtered if p.get("Difficulty", "").lower() == difficulty.lower()
        ]
    ...
    if topics:
        filtered = [
            p
            for p in filtered
            if any(topic.lower() in p.get("Topics", "").lower() for topic in topics)
        ]
    ...
    if min_days:
        filtered = [
            p
            for p in filtered
            if days_since_last_solve(p["Last Solved Date"]) >= min_days
        ]
...
def display_selected_problems(problems: List[Dict]):
    ...
    for i, problem in enumerate(problems, 1):
        ...
        if file_path:
            ...
            print("   ❌ File not found for this problem")
...
def show_stats(problems: List[Dict]):
    ...
    avg_days = sum(
        days_since_last_solve(p["Last Solved Date"]) for p in problems
    ) / len(problems)
...
def main():
    ...
    parser.add_argument("--min-days", type=int, help="Minimum days since last solve")
    parser.add_argument(
        "--balanced",
        action="store_true",
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Show statistics about selected problems"
    )
    ...
    if selected_problems:
        ...
        print("📝 Your progress will be automatically tracked!")
